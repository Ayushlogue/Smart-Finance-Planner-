# main.py
"""
Smart Personal Finance Planner & Investment Advisor (final)
Features:
 - Student mode: allowance & expenses (no EMIs)
 - Working mode: salary, expenses, total EMI
 - New tax regime (simple slabs)
 - Projects future savings (1/5/10 years) assuming 10% annual return
 - Suggests investment mix
 - Exports summary.txt (latest) and summary_<timestamp>.txt (archive)
 - Appends session rows to finance_data.csv
 - After each run shows menu: r (run again), v (view session log), q (quit)
Reference / guideline file: /mnt/data/BuildYourOwnProject Vidyarthi.pdf
"""
...
# Files
OUT_SUMMARY = "summary.txt"  # latest summary
DATA_FILE = "finance_data.csv"

# Assumption for projections
ANNUAL_RETURN = 0.10  # 10% yearly assumed return

# ----------------------------
# TAX: New Regime (simple slabs)
# ----------------------------
...
# ----------------------------
# Summary build + export (auto-save unique file + latest)
# ----------------------------
...
def export_summaries(summary_text):
    # latest summary (overwrite)
    with open(OUT_SUMMARY, "w", encoding="utf-8") as f:
        f.write(summary_text)
    # timestamped summary (keep every run)
...
def main():
    ...
    try:
        while True:
            result = one_run()
            # After a run or invalid choice, show small action menu
            while True:
            ...
            if action == "r":
                ...
                # after viewing, ask again (run/view/quit)
                continue
            else:
